...
func SetupRouter(cfg *config.Config, dbConn *gorm.DB, rdbClient *redis.Client) *gin.Engine {
	...
	api := r.Group("/api/v1")
	{
		...
		{
			authGroup.POST("/signup", handler.NewAuthHandler(dbConn, cfg).SignupHandler)
			authGroup.POST("/login", handler.NewAuthHandler(dbConn, cfg).LoginHandler)
		}

		protected := api.Group("")
		protected.Use(middleware.AuthMiddleware(cfg))
		{
			protected.GET("/profile", handler.NewAuthHandler(dbConn, cfg).ProfileHandler)
			...
			protected.POST("/users", middleware.RBACMiddleware(dbConn, "users", "create"), handler.CreateUser(dbConn))
			protected.PUT("/users/:id", middleware.RBACMiddleware(dbConn, "users", "update"), handler.UpdateUser(dbConn))
			protected.DELETE("/users/:id", middleware.RBACMiddleware(dbConn, "users", "delete"), handler.DeleteUser(dbConn))

			protected.GET("/audit/stream", handler.AuditStreamHandler())

			protected.GET("/setting", middleware.RBACMiddleware(dbConn, "settings", "read"), handler.GetSetting(dbConn))
		}
	}
